...
~~~
    ...
    implementation 'com.github.trongluan99:Mia-Studio:1.0'
...
# <a id="setup_Miaad"></a>Setup MiaAd
...
class App extends AdsMultiDexApplication(){
    ...
    public void onCreate() {
        super.onCreate();
    ...
        String environment = BuildConfig.build_debug ? MiaAdConfig.ENVIRONMENT_DEVELOP : MiaAdConfig.ENVIRONMENT_PRODUCTION;
        ...
        miaAdConfig.setAppsflyerConfig(appsflyerConfig);
    
        // Optional: setup client token SDK Facebook
        miaAdConfig.setFacebookClientToken(FACEBOOK_CLIENT_TOKEN)
...
~~~ 
    MiaAdCallback adCallback = new MiaAdCallback() {
        @Override
...
~~~
    MiaAd.getInstance().setInitCallback(new MiaInitCallback() {
            @Override
            public void initAdSuccess() {
                MiaAd.getInstance().loadSplashInterstitialAds(SplashActivity.this, idAdSplash, TIME_OUT, TIME_DELAY_SHOW_AD, true, adCallback);
            }
        });    
~~~
...
~~~
    private fun loadInterCreate() {
      ApInterstitialAd mInterstitialAd = MiaAd.getInstance().getInterstitialAds(this, idInter);
    }
~~~
...
~~~
    <com.ads.control.ads.bannerAds.MiaBannerAdView
...
~~~~
    MiaAd.getInstance().loadBanner(this, idBanner);
~~~~
...
~~~~
    MiaAd.getInstance().loadNativeAdResultCallback(this,ID_NATIVE_AD, com.ads.control.R.layout.custom_native_max_small,new MiaAdCallback(){
            @Override
...
~~~~
    MiaAd.getInstance().populateNativeAdView(MainApplovinActivity.this,nativeAd,flParentNative,shimmerFrameLayout);
~~~~
...
~~~~
    <com.ads.control.ads.nativeAds.MiaNativeAdView
        android:id="@+id/MiaNativeAds"
        android:layout_width="match_parent"
...
~~~~
    MiaNativeAdView.loadNativeAd(this, idNative);
~~~~
...
~~~~
    ...
    MiaAdAdapter adAdapter = MiaAd.getInstance().getNativeRepeatAdapter(this, idNative, layoutCustomNative, com.ads.control.R.layout.layout_native_medium, originalAdapter, listener, 4);
    
    // Ad native fixed in position
    MiaAdAdapter adAdapter = MiaAd.getInstance().getNativeFixedPositionAdapter(this, idNative, layoutCustomNative, com.ads.control.R.layout.layout_native_medium, originalAdapter, listener, 4);
...
~~~~
    ApRewardAd rewardAd = MiaAd.getInstance().getRewardAd(this, idAdReward);
...
~~~~
    override fun onCreate() {
      ...
      miaAdConfig.setIdAdResume(AppOpenManager.AD_UNIT_ID_TEST);
      ...
    }
~~~~
